...
editButtons.forEach(function (editbtn) {
  editbtn.addEventListener('click', function() {
	...
	xhr.open("GET", domainName+"/assets/doc/"+fileValue+".php", true);
...
document.getElementById('documents').addEventListener('click', function() {
	...
	document.getElementById('files-wrap').classList.remove('main-hidden');
});

//For saving editer text into files
let savingText = document.getElementById('saving-text');
quillEditor.on('text-change', function() {
	savingText.classList.add('on');
	let getQuillText = document.querySelector(".ql-editor").innerHTML;
	document.getElementById("savetext").innerText = getQuillText;
    document.getElementById("save-btn").click();
	document.getElementById('saveframe').addEventListener('load', function(event) {
		savingText.classList.remove('on');
	});
});

//Input Filename file
let enterFileName = document.getElementById('filename');
enterFileName.addEventListener('input', function() {
	enterFileName.value = enterFileName.value.replace(/\W+/g, ' ');
});
